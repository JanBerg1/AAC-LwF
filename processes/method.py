...
def _do_training(model: Module,
    ...
    def train():
        # Log starting time
        nonlocal model, validation_metric, prv_validation_metric, patience_counter, convergence, early_stopping_dif, metrics_list
        start_time = time()
        ...
        if settings_data['use_validation_split']: #and batch % (50 * 12/settings_data["batch_size"]) == 0:
            ...
            val_loss_str = '--'
            if not settings_data['use_validation_split']:
        ...
        if settings_training['evaluate_after_each']: #and batch % (50*12/settings_data["batch_size"]) == 0:
    ...
    file = open(f'metrics_{settings_training["lwf_weight"]}_{settings_data["batch_size"]}.p', "wb+")
